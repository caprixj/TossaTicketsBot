...
from model.database.materials import Material
...
def to_iso_z(dt: Union[datetime, str]) -> str:
...
def to_kyiv_str(dt: Union[datetime, str]) -> str:
    if isinstance(dt, str):
        dt = datetime.fromisoformat(dt.replace('Z', '+00:00'))
...
def get_formatted_name(member: Union[Member, DelMember], ping: bool = False) -> str:
    ...
    return f"[{_escape_brackets(name)}](tg://user?id={member.user_id})" \
        if ping else escape_markdown_v2(name)
...
def struck_html(text: str) -> str:
    return f'<s>{text}</s>'


async def callback_struck_html(callback: CallbackQuery):
    await callback.message.edit_text(
        text=struck_html(callback.message.text),
